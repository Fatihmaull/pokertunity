...
**A statically prerendered page freezes its environment at build time.** `next build` evaluates page modules to collect page data, so anything a page reads from `process.env` at module load — `MATCH.handCap` via `HAND_CAP`, most obviously — is baked into the HTML inside the Docker build stage, where only `DATABASE_URL` and `NEXT_TELEMETRY_DISABLED` are set. The page then ships that value and the edge serves it for as long as its `s-maxage` says. This has already shipped once: `/matches` told visitors the game was 100 hands while every match ran 30. A page whose copy depends on configuration must either read it in a server component and pass it down, or opt out of prerendering; adding the variable to the build stage looks like a fix and only moves the disagreement.
